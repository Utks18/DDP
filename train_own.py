...
from utils.validations import validate_2 as validate
...
from utils.trainers import train_mlrs
...
def main():
    ...
    attn_group = {'params': attn_params}
    print('num of params in attention: ', len(attn_params))
    sgd_polices = [attn_group]
    ...
    try:
        classifier_params = model.classifier_params()
    except:
        classifier_params = model.module.classifier_params()
    print('num of params in attn layer: ', len(classifier_params))
    classifier_grp = {'params': classifier_params}
    sgd_polices.append(classifier_grp)
    ...
    for epoch in range(args.start_epoch, cfg.OPTIM.MAX_EPOCH):
        batch_time, losses, mAP_batches = train_mlrs(train_loader, [val_loader],  model, optim, sched, args, cfg, epoch)
        print('Train: [{0}/{1}]\t'
