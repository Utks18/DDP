...
from utils.validations import validate, validate_zsl, validate_2
...
def train_mlrs(data_loader, val_loaders, model, optim, sched, args, cfg, epoch, cls_id=None):
    batch_time = AverageMeter()
    mAP_batches = AverageMeter()
    losses = AverageMeter()
    Softmax = torch.nn.Softmax(dim=1)
    ...
    if not isinstance(model, nn.DataParallel):
        ...
        model.module.cross_attention.train()
    ...
    for i,   (images, target) in enumerate(data_loader):
        ...
        # compute output
        with autocast():
            output_pos, output_neg = model(images, batch_cls_id_input)
        # loss = args.loss_w * criterion(output, target)
        if cls_id is not None:
            # output = output[:, :, cls_id['train']]
            # target = target[:, cls_id['train']]
        ...
        loss_pos = criterion(output_pos.float(), target)
        loss_neg = criterion(output_neg.float(), (~target.bool()).float())
        ...
        losses.update(loss.item(), images.size(0))
        pred = output_pos
        mAP_value = mAP(target.cpu().numpy(), pred.detach().cpu().numpy())
        mAP_batches.update(mAP_value, images.size(0))
        batch_time.update(time.time()-end)
        end = time.time()
        if i % args.print_freq == 0:
